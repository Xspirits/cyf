...
riotApi  = new RiotApi({host: 'prod.api.pvp.net',path: '/api/lol/',key: auth.leagueoflegend.key,debug: true,secure:true});
...
exports.postTwitter = (accessToken, message, callback) ->
  ...
  r = request.post(
    ...
    oauth: params
  , (err, resp, body) ->
    console.error("Error occured: ", err)  if err
    body = JSON.parse(body)
    console.error("Error returned from  twitter: ", body.error)  if body.error
    console.log body
